...
for xcTeam in range(1000):
	...
	ngentot2 = f"Dalvik/2.1.0 (Android 9; L-03K Build/PKQ1.{str(rr(111111,199999))}.001) [FBAN/MessengerLite;FBAV/{str(rr(120,150))}.0.0.2.{str(rr(110,150))};FBPN/com.facebook.mlite;FBLC/en_US;FBBV/{str(rr(200000000,299999999))};FBCR/Airtel;FBMF/Facebook;Facebook/lge;FBDV/L-03K;FBSV/9;FBCA"
...
class Menu:
	...
	def menu(self):
		...
		yonkou.append(Panel(f" {K2}Author   {P2}: {H2}JANXD\n {K2}Github  {P2} : {H2}https://github.com/JANSXD\n{K2} Facebook {P2}: {H2}VaniGemes\n{K2} Whatsapp {P2}: {H2}+6283841160198",width=43,padding=(0,2),title=f"{M2}• {H2}• {K2}• {K2}Info-Author {M2}• {H2}• {K2}•",style=f"{color_panel}"))
		...
		if menu in["logout"]:
			...
			exit(prints(Panel(f"""{H2}berhasil menghapus cookie, silahkan ketik ulang python JansXD-fb.py""",width=87,style=f"{color_panel}")))
		###----------[ ID PUBLIK ]---------- ###
		elif menu in["1","01"]:
